...
export default function AdminPage() {
  ...
  const fetchAssets = async () => {
    try {
      const { data, error } = await supabaseAdmin.storage.from('assets').list('', {
        limit: 100,
        offset: 0,
      })
      ...
      const assetsList = data?.map(file => ({
        name: file.name,
  ...
  const deleteAsset = async (assetName: string) => {
    try {
      const { error } = await supabaseAdmin.storage.from('assets').remove([assetName])
      if (error) {
